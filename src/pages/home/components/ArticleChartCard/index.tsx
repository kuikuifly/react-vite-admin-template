  ...
  return (
    <Spin spinning={loading} size='large'>
      <Card
        ...
        title={'page.home.articlechartcard.card-title'}
        extra={<Tag color='cyan'>{'page.home.text-day'}</Tag>}
      >
        <div className={styles.num}>{visitData.num.toLocaleString()}</div>
        <div className={styles.height40}>
          <div className={styles.articleText}>
            <span>
              {'page.home.text-daycompare'} {Math.abs(visitData.day)}%
              {visitData.day > 0 ? (
            ...
            <span className='margin-l10'>
              {'page.home.text-weekcompare'} {Math.abs(visitData.week)}%
              {visitData.week > 0 ? (
                <span className={styles.colored4014}>
                  <IconSvg name='arrow-down' style={{ transform: 'rotate(180deg)' }} />
                </span>
        ...
        <Divider />
        <Row>
          <Col span={12}>{'page.home.text-total'}</Col>
